...
class FixedIndividualIdentityDemo {
  ...
  async testFixedIndividualAccess(groupName, eoaName, operation, value = null) {
    ...
    try {
      ...
      if (operation === "write") {
        ...
        if (receipt?.success) {
          console.log(`✅ ${eoaName} WRITE SUCCESSFUL - Paladin allowed the transaction`);
          console.log(`   📝 Transaction Hash: ${receipt.transactionHash || 'N/A'}`);
          return true;
        } else {
          console.log(`❌ ${eoaName} WRITE FAILED - Paladin rejected the transaction`);
          console.log(`   📝 Receipt: ${JSON.stringify(receipt)}`);
          return false;
        }
      } else {
        console.log(`   🔄 Attempting READ call...`);
        const result = await contractWithIndividualIdentity.call({
          from: identity.verifier.lookup,
          function: "retrieve"
        });

        // 🚨 CRITICAL: This means Paladin ALLOWED the call - no error was thrown
        console.log(`🚨 PALADIN ALLOWED READ for ${eoaName} - This is concerning if unauthorized!`);
        console.log(`   📖 Retrieved Value: ${result.value}`);
        console.log(`   🔍 Is this supposed to be blocked? ${!isAuthorizedMember ? "YES - BUG DETECTED!" : "No - Expected behavior"}`);
        
        if (!isAuthorizedMember) {
          console.log(`🐛 POTENTIAL BUG: Unauthorized ${eoaName} was able to read private data!`);
          console.log(`   💡 This suggests endorsementType: 'group_scoped_identities' may not be working`);
        }
      ...
      // Error thrown = Paladin actually blocked the request
      ...
      if (error.message.includes("500") || error.message.includes("Privacy group") || error.message.includes("not found")) {
        console.log(`   📍 Error Type: Privacy group not accessible from this node`);
      } else if (error.message.includes("unauthorized") || error.message.includes("permission")) {
        console.log(`   📍 Error Type: Identity-level authorization failure`);
      } else {
        console.log(`   📍 Error Type: Other blocking mechanism`);
      }
      
      if (isAuthorizedMember) {
        console.log(`❌ ${eoaName} UNEXPECTED BLOCK - This should be authorized!`);
        console.log(`🐛 POTENTIAL BUG: Authorized member was blocked`);
      } else {
        console.log(`✅ ${eoaName} CORRECTLY BLOCKED - Security working as expected`);
      }
      return false;
    }
  }
  ...
  async runDemo() {
    try {
      ...
      console.log("\n🔥 CRITICAL TEST: Unauthorized EOAs attempting WRITE to GROUP1_INDIVIDUAL_FIXED:");
      console.log("Testing if unauthorized EOAs can actually modify the contract state...");
      
      // Test writes from unauthorized users
      for (const eoaName of ["EOA2", "EOA4", "EOA5", "EOA6"]) {
        console.log(`\n⚠️ Testing unauthorized write from ${eoaName}:`);
        await this.testFixedIndividualAccess("GROUP1_INDIVIDUAL_FIXED", eoaName, "write", 999);
      }
      ...
      console.log("\n🔥 CRITICAL TEST: Unauthorized EOAs attempting WRITE to GROUP2_INDIVIDUAL_FIXED:");
      console.log("Testing if unauthorized EOAs can actually modify the contract state...");
      
      // Test writes from unauthorized users  
      for (const eoaName of ["EOA2", "EOA3", "EOA5", "EOA6"]) {
        console.log(`\n⚠️ Testing unauthorized write from ${eoaName}:`);
        await this.testFixedIndividualAccess("GROUP2_INDIVIDUAL_FIXED", eoaName, "write", 888);
      }
      ...
      await this.testFixedIndividualAccess("GROUP1_INDIVIDUAL_FIXED", "EOA1", "read");
      await this.testFixedIndividualAccess("GROUP2_INDIVIDUAL_FIXED", "EOA1", "read");
